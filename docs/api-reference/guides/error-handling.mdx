...
The `202` response includes the identifier used by the worker and sync-status endpoints:
...
`batch_id` is the same value returned as `run_id` by `/api/v1/users/{user_id}/sync/runs` and in sync-status events. If the worker drops individual records, the terminal run has status `partial`; its `metadata` contains the saved counts, `dropped_count`, and PII-free validation errors.

An HTTP `202` only confirms that the batch was queued. Clients must follow the
matching `batch_id` until a terminal run is reported; `partial`, failed, or
missing terminal status must not be treated as a successful import.
