...
| `status`          | `SyncStatus`          | One of `in_progress`, `success`, `partial`, `failed`, `cancelled`, `skipped`. Terminal statuses end a run.          |
...
| `metadata`        | `object`              | Run-specific counts and structured error details. SDK partial runs include `dropped_count` and PII-free `errors`.  |
...
For SDK uploads, `run_id` equals the `batch_id` returned by the `202 Accepted` response. A run with dropped records is returned with `status: "partial"`, a non-null `error` summary, and terminal-event `metadata` containing `records_saved`, `workouts_saved`, `sleep_saved`, `dropped_count`, and PII-free validation errors. A terminal `success` run has no dropped records.
