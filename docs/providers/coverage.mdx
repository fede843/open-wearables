...
<Accordion title="Google Health — Health Connect (mobile SDK)">
  ...
  **Integration Method:** Push-based via Android SDK (no OAuth)
  ...
  - Workout sessions carry duration, type, start/end times, supported aggregates, and optional route/segment/HR-zone details; in-workout samples remain pending
  ...
  - No health scores; SDK workout routes are persisted as canonical `latitude`/`longitude` series with optional `elevation`, and `segments`/`hrZones` are stored in workout details
  - Route values use `NUMERIC(15,6)` storage and an idempotent source/type/timestamp upsert, so repeated raw timestamps do not create duplicate series rows
  - Standalone `LATITUDE`/`LONGITUDE` metric records are intentionally not mapped; `workout.route` is the canonical GPS path and avoids duplicate points
  - In-workout `samples` remain accepted-but-unprocessed; existing sleep-stage processing is unchanged and sleep sample extensions remain pending
</Accordion>
